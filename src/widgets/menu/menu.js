...
import {Out, render} from 'generated/cutejs/oizombie/widgets/menu/menu.jst';
...
export default class Menu extends AbstractCuteWidget {
	...
	activateItemByID(input) {
		...
		if(this.itemToUnselect){
			this.itemToUnselect.setSelected(false);
		}
		...
		if (subId !== null) {
			...
			if(this.previousSubItems){
				this._hideSubItems(this.previousSubItems);
			}
	...
	setMenuSection(param) {
		...
		if (!param.status) {
			...
			this.previousSubItems = this._menuItemId
		} else {
	 ...
	 */
	_generateMenu(menu) {
		console.log('_generateMenu', menu);
		this._menuItemId = null;
		this._menuItems = [];
		...
		for (const prop in menu) {
			...
			if (item.contentLists) {
				item.contentLists = item.contentLists;
			}
			...
			if (item.subItems) {
				...
				item.subItems.forEach((subitem, i) => {
					...
					if (item.contentLists) {
						subitem.contentLists = item.contentLists;
					}
